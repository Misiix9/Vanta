<script lang="ts">
  ...
  let selected = $derived(filtered.find((i) => i.id === selectedId) ?? filtered[0] ?? null);
  ...
  function handleKeydown(e: KeyboardEvent) {
    ...
    if (e.key === "ArrowDown") {
      ...
      selectedId = filtered[next]?.id ?? null;
      scrollToItem(next);
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      const prev = Math.max(idx - 1, 0);
      selectedId = filtered[prev]?.id ?? null;
      scrollToItem(prev);
...
<div class="cv-root">
  <div class="cv-search">
    ...
    <input
      ...
      autofocus
    />
    <span class="cv-count">{filtered.length}</span>
  </div>
